...
if __name__ == "__main__":
    ...
    for idx, learnRate in enumerate(learnRates):
        w = tf.Variable(tf.truncated_normal([d, numClass], dtype=tf.float64), name="weights")
    ...
    plt.close('all')
    fig = plt.figure(1)
    plt.scatter(range(epochs), training_losses[bestLearnRateIdx], marker='.')
    plt.xlabel('the n-th epoch')
    plt.ylabel('cross-entropy loss')
    plt.title("cross-entropy loss vs number of epoch for learning rate of %f" % learnRates[bestLearnRateIdx])
    fig.savefig("part2_2_1_training_loss.png")
    fig = plt.figure(2)
    plt.scatter(range(epochs), training_accuracies[bestLearnRateIdx], marker='.')
    plt.xlabel('the n-th epoch')
    plt.ylabel('accuracy')
    plt.title("accuracy vs number of epoch for learning rate of %f" % learnRates[bestLearnRateIdx])
    fig.savefig("part2_2_1_training_accuracy.png")
    fig = plt.figure(3)
    plt.scatter(range(epochs), valid_losses[bestLearnRateIdx], marker='.')
    plt.xlabel('the n-th epoch')
    plt.ylabel('cross-entropy loss')
    plt.title("cross-entropy loss vs number of epoch for learning rate of %f" % learnRates[bestLearnRateIdx])
    fig.savefig("part2_2_1_validation_loss.png")
    fig = plt.figure(4)
    plt.scatter(range(epochs), valid_accuracies[bestLearnRateIdx], marker='.')
    plt.xlabel('the n-th epoch')
    plt.ylabel('accuracy')
    plt.title("accuracy vs number of epoch for learning rate of %f" % learnRates[bestLearnRateIdx])
    fig.savefig("part2_2_1_validation_accuracy.png")
